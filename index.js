...
function parseSales(text) {
  ...
  return {
    ...
    qty: num(t.match(/(?:套餐數|總鍋數)\s*:\s*([\d,]+)/)?.[1]),
    fp: t.match(/外場薪資\s*:\s*([\d,]+).*?([\d.]+)%/)?.slice(1).map(num) || [0,0],
    bp: t.match(/內場薪資\s*:\s*([\d,]+).*?([\d.]+)%/)?.slice(1).map(num) || [0,0]
  };
}
...
外場：${p.fp[0]}（${p.fp[1]}%）
內場：${p.bp[0]}（${p.bp[1]}%）
總計：${p.fp[0] + p.bp[0]}（${(p.fp[1] + p.bp[1]).toFixed(2)}%）`;
